...
# Make sure  mock_gene_expression directory
if (basename(getwd()) != "mock_gene_expression") {
  setwd("resources/mock_gene_expression")
}
...
generate_mock_expression_matrix <- function(num_genes = 200,
  ...
  colnames(expression_matrix) <- c(
    paste0("Control", 1:samples_per_group),
...
gene_wise_log_t_test <- function(expression_matrix) {
  ...
  expression_matrix %>%
    ...
    group_by(Gene) %>%
    summarise(
      log2FC = log2(mean(Expression[Group == "Treatment"]) / mean(Expression[Group == "Control"])), # nolint: line_length_linter.
...
write_csv(as.data.frame(experiment1), "experiment1.csv")
...
write_csv(as.data.frame(experiment2), "experiment2.csv")
